...
const edit = ( props) => {
console.log(props.data.Hobbies.includes("Cricket"))
  // useEffect(() => {
    //     // console.log(props.data[0].hobbies)
    //      setName(props.data.Name)
    //       setId(props.data.ID)
    //       setCourse(props.data.Course)
    //       setMob(props.data.Mob)
    //       setPicURL(props.data.img)
    //       setDOB(props.data.DOB)
    //       setEmail(props.data.Email)
    //       setGender(props.data.Gender)
    //       setFee(props.data.Fee)
    //       setHobbies(props.data.hobbies)
    //       setScores(props.data.scores)
    //     setTableScores({...props.data.scores,edit:<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal2" onClick={(event)=>{editHandler(props.data.scores)}}></button>})
    ...
    let obj2={
            ...
            prac2:obj.prac2,
            total:obj.total,
            edit:<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal2" onClick={(event)=>{editHandler(obj1)}}></button>
  ...
  const [Mob, setMob] = useState(props.data.Mob);
  ...
  const [picURL, setPicURL] = useState(props.data.pic);
  ...
  const [prac1,setprac1]=useState('')
  const [prac2,setprac2]=useState('')
  const [total,settotal]=useState('')
  const [scores,setScores]=useState(props.data.scores)
  const [tablescores,setTableScores]=useState(tablerows)
